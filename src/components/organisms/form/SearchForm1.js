...
const SearchForm = props => {
  const { title, searchInfo, onChange, onSubmit } = props;
  const [state, setState] = useState(searchInfo);
  const [searchParam, setSearchParam] = useState({});
  const searchState = window.sessionStorage.getItem('searchState');
  const [hide, setHide] = useState(searchState === 'true');
  ...
  useEffect(() => {
    window.sessionStorage.setItem('searchState', hide);
  }, [hide]);
  ...
  return (
    <aside id="mAside" className={`${styles.aside}${hide ? ` ${styles.hide}` : ''}`}>
      {hide && (
        <button type="button" className={styles.show_button} title="검색창 펼치기" onClick={() => setHide(false)}>
